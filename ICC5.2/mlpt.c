...
int vaddr_to_paddr(int dir_page_num, int vaddr)
{
    ...
    int page_table_entry = get_page_table_entry(dir_page_num, dir_index);
    if(page_table_entry == 0){
        return 0;
    }

    int physical_page = get_page_table_entry(page_table_entry, page_table_index);
    if(physical_page == 0){
        return 0;
    }

    return get_page_start_addr(physical_page) + offset;
}
