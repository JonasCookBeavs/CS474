...
void test_open_directory(void)
{
    ...
    CTEST_ASSERT(dir != NULL, "directory should not be null");
    CTEST_ASSERT(dir->offset == 0, "directory offset should be zero after opening");
    directory_close(dir);

    cleanup_test_image();
}

void test_get_directory(void)
{
    setup_test_image();
    mkfs();
    ...
    CTEST_ASSERT(dir_call == 0, "first call of directory_get should return 0");
    CTEST_ASSERT(ent.inode_num == 0, "first entry inode_num should be 0");
    CTEST_ASSERT(strcmp(ent.name, ".") == 0, "first entry name should be '.'");

    int dir_call2 = directory_get(dir, &ent);
    CTEST_ASSERT(dir_call2 == 0, "second call of directory_get should return 0");
    CTEST_ASSERT(ent.inode_num == 0, "second entry inode_num should be 0");
    CTEST_ASSERT(strcmp(ent.name, "..") == 0, "second entry name should be '..'");

    int dir_call3 = directory_get(dir, &ent);
    CTEST_ASSERT(dir_call3 == -1, "third call of directory_get should return -1");

    directory_close(dir);
    cleanup_test_image();
}

void test_close_directory(void)
{
    setup_test_image();
    mkfs();
    ...
    CTEST_ASSERT(true, "directory_close should not crash");

    cleanup_test_image();
}

void test_made_directory_is_findable(void)
{
    setup_test_image();
    mkfs();
    ...
    CTEST_ASSERT(found == 1, "foo should be in root directory once created");
...
// --- namei() ---
void test_namei_returns_non_null_for_slash(void)
{
    setup_test_image();
    mkfs();

    struct inode *name = namei("/");
    CTEST_ASSERT(name != NULL, "/ should not return NULL");
    
    iput(name);
    cleanup_test_image();
}

void test_namei_returns_correct_inode(void)
{
    setup_test_image();
    mkfs();

    directory_make("/foo");
    struct inode *name = namei("foo");
    CTEST_ASSERT(name != NULL, "/foo should not return NULL");
    CTEST_ASSERT(name->inode_num != 0, "foo should not be the root");

    iput(name);
    cleanup_test_image();
}

void test_namei_returns_null_for_invalid_path(void)
{
    setup_test_image();
    mkfs();

    struct inode *name = namei("/doesnotexist");
    CTEST_ASSERT(name == NULL, "doesnotexist should not exist if not created");
...
int main(void)
{
    ...
    test_made_directory_is_findable();

    test_namei_returns_non_null_for_slash();
    test_namei_returns_correct_inode();
    test_namei_returns_null_for_invalid_path();
