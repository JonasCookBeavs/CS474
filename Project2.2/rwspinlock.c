...
#define WRITE_LOCK_BIT (1 << 31)

_Atomic(int) reader_count;
_Atomic(int) lock_state = 0;
int writer_count;
...
{
    while(1) {
        int state = atomic_fetch_add(&lock_state, 1);
        if (!(state & WRITE_LOCK_BIT))
            break;
        atomic_fetch_add(&lock_state, -1);
    }
}
...
{
    int expected = 0;
    while(!atomic_compare_exchange_weak(&lock_state, &expected, WRITE_LOCK_BIT))
        expected = 0;
}
...
void write_unlock()
{
    atomic_fetch_and(&lock_state, ~WRITE_LOCK_BIT);
}
